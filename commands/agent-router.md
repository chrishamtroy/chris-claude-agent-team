...
2. **스킬 우선 라우팅 테이블** 매칭이 있으면 → 에이전트 스폰 대신 **Skill tool로 호출**
3. 매칭이 없으면 → 이 스킬을 무시하고 직접 처리
4. 사용자가 "직접 해" 또는 "에이전트 없이"라고 하면 → 라우팅 스킵
5. 여러 매칭이 있으면 → 가장 구체적인 것 선택 (스킬 > 에이전트)
6. 에이전트/스킬 호출 시 사용자의 원본 요청을 그대로 전달 (요약/변형 금지)

## 스킬 우선 라우팅 테이블 (에이전트보다 우선)

| 키워드 (하나라도 포함 시) | 호출 스킬 |
|------------------------|---------|
| 기획해줘, PRD 만들어줘, 제품 기획, 기능 명세, 유저 플로우, MVP 기획, 출시 계획, 사용자 리서치, JTBD, RICE, North Star, Working Backwards, PR/FAQ, Lean Canvas, 새 프로덕트, 새 SaaS, 새 앱 아이디어 | **product-planning** |
| 브레인스토밍, 아이디어 발산, 토론, 의사결정 도와줘 | brainstorm |
| 보안 리뷰, 보안 검토, OWASP, CWE, STRIDE | security-review |
...
| 제품 전략, 사업 전략, 로드맵, BMC, 전략 수립, 시장 분석, 경쟁 분석, 포지셔닝 (단발성 분석) | product-strategist |
